...
仕様書 9章の方針に沿い、MVPは「直接パース」区分の自治体を対象とする。

- 神奈川県（CSV, 疾患ごと・年ごと配布, 保健所管区別）※本物のオープンデータ
- 千葉市（CSV, 市全体）
- 北九州市（CSV, 市全体）
- 京都府（CSV, 発生地図用データ, 保健所管区別）※本物のオープンデータ
- 沖縄県（Excel, 保健所別）
- 山梨県（CSV, 県全体のみ）※本物のオープンデータ
...
**パイプライン動作確認用のサンプルデータ**を置き、Raw層の代わりとしている。
神奈川県・京都府・山梨県は実際に取得した本物のオープンデータ、
千葉市・北九州市・沖縄県は引き続き架空のデータ。
実データを使う場合は `backend/app/seed.py` の `SOURCES` を実URL取得結果に差し替える。

なお神奈川県・京都府は、SPEC.md作成時点の想定フォーマット（単純なCSV/素のHTML表）
と実際の配布形式が異なっていた。神奈川県は疾患ごと・年ごとに別ファイルで配布され、
患者数（実数）は含まれず定点当たり報告数のみ。京都府は地図ページのHTML表ではなく、
同ページからリンクされた全疾患×12管区の集計CSVの方が扱いやすかった。詳細は各
`backend/app/extract/*.py` のdocstring参照。
